...
const iconPaths: Record<string, string> = {
  ...
  alimentacao:
    "M12 8.25v-1.5m0 1.5c-1.355 0-2.697.056-4.024.166C6.845 8.51 6 9.473 6 10.608v2.513m6-4.871c1.355 0 2.697.056 4.024.166C17.155 8.51 18 9.473 18 10.608v2.513M15 8.25v-1.5m-6 1.5v-1.5m12 9.75l-1.5.75a3.354 3.354 0 01-3 0 3.354 3.354 0 00-3 0 3.354 3.354 0 01-3 0 3.354 3.354 0 00-3 0 3.354 3.354 0 01-3 0L3 16.5m18-4.5a9 9 0 11-18 0",
  transporte:
...
// Map partial matches for flexibility
...
function resolveIconKey(name: string): string {
  ...
  // Exact match
  if (iconPaths[normalized]) return normalized;
...
export function CategoryIcon({ name, className = "w-4 h-4", ...props }: CategoryIconProps) {
  const key = resolveIconKey(name);
  const d = iconPaths[key] ?? iconPaths.outros;
