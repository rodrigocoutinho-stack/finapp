    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-slate-50 rounded-xl p-4 text-center">
          <p className="text-xs font-medium text-slate-600 mb-1">Valor nominal</p>
          <p className="text-lg font-bold text-slate-700">{formatBRL(result.futureNominal)}</p>
        </div>
        <div className="bg-rose-50 rounded-xl p-4 text-center">
          <p className="text-xs font-medium text-rose-600 mb-1">Poder de compra real</p>
          <p className="text-lg font-bold text-rose-700">{formatBRL(result.futureReal)}</p>
        </div>
        <div className="bg-red-50 rounded-xl p-4 text-center">
          <p className="text-xs font-medium text-red-600 mb-1">Perda de poder de compra</p>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4">
        <h3 className="text-sm font-medium text-slate-700 mb-4">Valor real ao longo do tempo</h3>
        <div className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={result.yearlyData.slice(1)}>
              ...
              />
              <Legend />
              <Bar dataKey="nominalValue" name="Valor nominal" fill="#94a3b8" radius={[4, 4, 0, 0]} />
              <Bar dataKey="realValue" name="Poder de compra" fill="#f43f5e" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
